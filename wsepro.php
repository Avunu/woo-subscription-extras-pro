...
function wsepro_is_premium() {
    return true;
}

add_filter('mh_wse_settings', 'wsepro_settings');
function wsepro_settings($arr) {
    $arr['enable_background_update'] = array(
    ...
    $arr['background_process_per_run'] = array(
...
function wsepro_after_init() {
    // check background process abort
    if ( !empty($_GET['wsextra_abort_background']) ) {
        delete_option('wsextra_batch_price_update');
    }

    // check background process runner
...
add_action('wsextra_update_subscriptions', 'wsepro_update_subscriptions', 10, 1);
function wsepro_update_subscriptions($args) {
    $orders = wsextra_get_orders_to_update($args);

    // enqueue background proccess
    $args['processed'] = 0;
...
add_action('wp_ajax_woocommerce_save_variations', 'wsextra_save_variations', 10);
function wsextra_save_variations() {
    ...
    for ( $i = 0; $i <= $max_loop; $i++ ) {
        ...
        $new_price = !empty($sale_price) ? $sale_price : $regular_price;
    ...
    if ( !empty($data) ) {
        set_transient('wsextra_variations_change_'.$product_id, $data, 600);
    }
    else {
        delete_transient('wsextra_variations_change_'.$product_id);
    }
}
...
function wsepro_run_batch_price_update() {
    $batch = get_option('wsextra_batch_price_update');
    $runPerBatch = wsextra_option('background_process_per_run');
...
function wsepro_batch_progress() {
    $batch = get_option('wsextra_batch_price_update');
